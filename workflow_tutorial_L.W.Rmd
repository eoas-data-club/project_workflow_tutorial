...
library(tidyverse)
install.packages("ggbiplot")# for manipulating and plotting data
...
catch<-read.csv("catchrates.csv")
head(catch)
hist(catch$catchrate)
mean(catch$catchrate)
var(catch$catchrate)
...
ggplot(catch, aes(x=turbidity_fnu, y=catchrate))+
  geom_point()
...
ggplot(catch, aes(x=weather, y=turbidity_fnu))+
  geom_boxplot()

ggplot(catch, aes(x=total_dissolved_solids_mg.l, y=turbidity_fnu))+
  geom_point()
...
ggplot(catch, aes(x=total_dissolved_solids_mg.l, y= catchrate))+
  geom_point()+
  facet_wrap(~weather)

ggplot(catch, aes(x=turbidity_fnu, y= catchrate, color=weather))+
  geom_point()

ggplot(catch, aes(x=barometric_pressure_mmHg, y= catchrate))+
  geom_point()
```
...
cormat <- round(cor(catch[,-c(1:6)]), 2)
cormat
?cor()

...
data <- read.csv("catchrates.csv") 
catchrate_hist <- hist(data$catchrate) #add hist and trying again
...
plot(catchrate~turbidity_fnu, data = data) #check turbidity
...
par(mfrow = c(2, 2))
boxplot(catchrate~weather, data = data)
plot(data$catchrate~data$water_temp_c)
plot(data$catchrate~data$total_dissolved_solids_mg.l)
plot(data$catchrate~data$conductivity_muS.cm)
...
cormat <- cor(data[,7:ncol(data)])
library(reshape2)
melted_cormat <- melt(cormat)
ggplot(data = melted_cormat, aes(x=Var1, y=Var2, fill=value)) + 
 geom_tile()
library(GGally)
ggcorr(data[,7:ncol(data)], label = T)
```

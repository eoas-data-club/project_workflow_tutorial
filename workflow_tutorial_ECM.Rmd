...
hist(catchrates$air_temp_c)
hist(catchrates$water_temp_c)
hist(catchrates$do_mg.l)
hist(catchrates$percent_sat)
...
need to do a poisson regression (glm)

### Step 3: Can we see a relationship between turbidity and catch rate visually, without using any stats?
catch rate decreased as turbidity increased
**Use the function plot()**

```{r turbidity vs catch}
plot(catchrates$turbidity_fnu, catchrates$catchrate)
plot(catchrates$water_temp_c, catchrates$catchrate)
plot(catchrates$percent_sat, catchrates$catchrate)
plot(catchrates$total_dissolved_solids_mg.l, catchrates$catchrate)

```


### Step 4: Plot the relationship between other potential explanatory variables (weather, barometric pressure, water temperature, air temperature, time of day, etc.) and catch rates.
...
all of them! turbidity, temperature
need to do correlations to see what's important
